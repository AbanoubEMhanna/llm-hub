# LLM Hub — App Roadmap & Feature TODO

> **Vision:** The best local-first AI workbench. Combines **LM Studio's** model management and hardware awareness with **OpenCode's** code-first agent workflow, wrapped in a sharp **Linear/Vercel-inspired UI** that feels fast, focused, and professional.
>
...
- [x] OpenAI provider (cloud, API key stored in localStorage)
...
- [x] Model metadata (size, params, context, quantization)
...
- [~] **Model library browser** — browse & pull models without typing names (PR #17)
...
- [ ] Model sharing — export/import model configs
...
- [ ] Conversation sharing via link (with self-hosted server mode)
...
- [ ] Compare 3+ models in a grid layout
...
- [ ] 🔥 **Chat density preferences** — Compact / Comfortable / Spacious
...
- [ ] 🔥 **Shell command tool** — opt-in, with per-execution confirmation
...
- [ ] Code lens — inline AI suggestions on code blocks
- [ ] Test runner output display
- [ ] Copy code → file save dialog (write to disk)
- [ ] Multi-language REPL (run Python/JS in sandboxed env)
...
- [ ] Knowledge base stats (chunks, sources, last updated)
...
- [ ] 🔥 **PDF attachment** — extract and attach as context
...
- [x] Context usage indicator (warns >75%)
- [~] **GPU / VRAM monitoring** — live VRAM per loaded model (PR #13)
- [~] **Cloud API cost estimation** — per-model USD pricing (PR #12)
...
- [ ] 🔥 **Proper settings UI** — tabbed form controls instead of raw JSON (General · Providers · Tools · RAG · Audio · Advanced)
- [ ] 🔥 **Provider connection test** — ping + model count
...
- [ ] 🔥 **Prompt injection warning** — detect and flag suspicious injections
