...
| [`docker-compose.traefik.yml`](./docker-compose.traefik.yml) | Traefik (Docker labels) | Merge into `docker-compose.yml` if Traefik already runs on the same Docker network |
