...
function buildSpec(port) {
  return {
    ...
    tags: [
      ...
      { name: 'AuditLog', description: 'Local, opt-in security audit log of every tool/agent execution' },
      ...
      schemas: {
        ...
        RagCollection: {
          ...
          properties: {
            ...
            embeddingModel: { type: 'string', nullable: true, example: 'nomic-embed-text', description: 'Embedding model most recently used to embed this collection’s chunks, or null if unknown (created before this field existed).' },
            stale:          { type: 'boolean', description: 'True when embeddingModel differs from the currently configured rag.embedding_model. Re-embed via POST /v1/rag/collections/{id}/reembed before relying on search results.' },
          },
        },
        ToolStatus: {
          type: 'object',
    ...
    paths: {
      ...
      '/v1/rag/collections/{id}/reembed': {
        post: {
          tags: ['RAG'],
          summary: 'Re-embed a collection with the current embedding model',
          description: 'Re-runs embedding for every chunk in the collection using the currently configured `rag.embedding_model`, and updates the collection’s recorded embedding model. Use this after changing `rag.embedding_model` to fix a collection reported as `stale` by GET /v1/rag/collections — vectors from a different model live in a different embedding space and can’t be meaningfully compared.',
          parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }],
          responses: {
            200: {
              description: 'Re-embedding complete',
              content: {
                'application/json': {
                  schema: {
                    type: 'object',
                    properties: {
                      ...
                      chunksReembedded:  { type: 'integer', example: 42 },
                    },
                  },
                },
              },
            ...
            404: { description: 'Collection not found', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
            500: { description: 'Embedding request failed', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
          },
        },
      },
      '/v1/rag/query': {
      ...
      '/v1/audit-log': {
        get: {
          tags: ['AuditLog'],
          summary: 'Recent tool/agent execution audit entries',
          description: 'Security-focused record of every tool execution across all runAgentLoop provider branches, including cache hits and a success/failure verdict per call. Enable with `audit.enabled` in config; empty when disabled. Unlike `/v1/agent-runs`, this is not scoped to `/v1/chat` sessions and records every execution unconditionally.',
          parameters: [
            { name: 'limit', in: 'query', schema: { type: 'integer', default: 200, maximum: 1000 }, description: 'Max entries to return, most recent first' },
          ],
          responses: {
            200: {
              description: 'Audit log status and recent entries',
              content: {
                'application/json': {
                  schema: {
                    type: 'object',
                      ...
                      entries: {
                        ...
                        items: {
                          ...
                          properties: {
                            id:       { type: 'string' },
                            ts:       { type: 'string', format: 'date-time' },
                            tool:     { type: 'string', example: 'web_search' },
                            provider: { type: 'string', example: 'ollama' },
                            model:    { type: 'string', example: 'llama3.2:3b' },
                            args:     { type: 'string' },
                            result:   { type: 'string' },
        ...
        delete: {
          tags: ['AuditLog'],
          summary: 'Clear the local audit log',
          responses: {
            200: { description: 'Cleared', content: { 'application/json': { schema: { type: 'object', properties: { cleared: { type: 'boolean' } } } } } },
          },
        },
