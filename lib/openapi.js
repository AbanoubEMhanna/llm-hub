    ...
    paths: {
      ...
      '/v1/rag/query': {
        post: {
          ...
          description: 'Provide `collectionId` to search a single collection, or `collectionIds` to search several together. Omit both to search across every collection at once, with results merged and re-ranked by score.',
          requestBody: {
            required: true,
            content: {
              'application/json': {
                schema: {
                  ...
                  required: ['query'],
                  properties: {
                    collectionId:  { type: 'string', format: 'uuid', description: 'Search only this collection.' },
                    collectionIds: { type: 'array', items: { type: 'string', format: 'uuid' }, description: 'Search only these collections, merged and re-ranked together.' },
