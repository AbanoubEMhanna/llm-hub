...
function recommendModels(catalog, hardware, installedNames = [], limit = 3) {
  ...
  const picks = [];
  const general = bestOf(m => !m.tags?.includes('vision'));
  if (general) picks.push({ category: 'general', ...general });

  const code = bestOf(m => m.tags?.includes('code'));
  if (code && code.model.name !== general?.model?.name) picks.push({ category: 'code', ...code });

  const vision = bestOf(m => m.tags?.includes('vision'));
  if (vision) picks.push({ category: 'vision', ...vision });
