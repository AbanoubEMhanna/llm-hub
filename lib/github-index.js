 ...
 * `truncated` is set when the walk stopped early with more entries left
 * unvisited, so callers can tell a user "only part of this repo was indexed"
 * instead of silently reporting a full-looking result.
 *
 * @param {string} rootDir
 * @param {{maxFiles?: number, maxTotalBytes?: number, maxFileBytes?: number}} [opts]
 * @returns {{files: Array<{relPath: string, absPath: string, size: number}>, truncated: boolean}}
 */
function walkRepoFiles(rootDir, opts = {}) {
  ...
  function walk(dir, relDir) {
    if (results.length >= maxFiles || totalBytes >= maxTotalBytes) { truncated = true; return; }
    let entries;
 ...
 * command.
 *
 * `http.followRedirects=false` blocks git from silently following an
 * HTTP(S) redirect to a host `isAllowedRepoUrl` never saw and never checked
 * against the private-host deny-list. This doesn't close every SSRF vector
 * (a server that returns a private address directly, with no redirect, is
 * still only defended by `isAllowedRepoUrl`), but it removes the specific
...
function cloneRepo(repoUrl, destDir, opts = {}) {
  ...
  const args = ['-c', 'http.followRedirects=false', 'clone', '--depth', '1', '--single-branch'];
