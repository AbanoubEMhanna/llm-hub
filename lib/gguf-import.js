 ...
 * names are `name[:tag]`, optionally namespaced. `.`/`..` segments are
 * rejected even though `MODEL_NAME_RE` otherwise allows `.` and `/`, since
 * Ollama stores manifests under a path derived from the model name — a
 * segment of `..` sent through to `/api/create` would be a path-traversal
 * primitive against that manifest directory.
 *
 * @param {string} name
 * @returns {boolean}
 */
function isValidModelName(name) {
  ...
  if (!trimmed || !MODEL_NAME_RE.test(trimmed)) return false;
  return trimmed.split('/').every((seg) => seg !== '.' && seg !== '..');
}
...
function streamToFileWithHash(readable, destPath, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const cleanupAndReject = (err) => {
      ...
      // Wait for `out`'s fd to actually finish closing before unlinking —
      // destroy() only requests closure, and a write still in flight when we
      // call it can otherwise flush to disk (recreating the file) after the
      // fs.rm() below has already run, leaving the partial file behind.
      const unlink = () => fs.rm(destPath, { force: true }, () => reject(err));
      if (out.destroyed || out.closed) {
        unlink();
      } else {
        out.once('close', unlink);
        try { out.destroy(); } catch { unlink(); }
      }
    };
    ...
    readable.on('data', (chunk) => {
      bytes += chunk.length;
      if (bytes > maxBytes) {
        const err = new Error(`File exceeds the ${Math.round(maxBytes / (1024 * 1024 * 1024))}GB import limit`);
        err.code = 'GGUF_LIMIT_EXCEEDED';
        cleanupAndReject(err);
        return;
      }
      hash.update(chunk);
    });
    readable.on('end', () => { reachedEnd = true; });
    readable.on('error', cleanupAndReject);
    // A client that disconnects mid-upload destroys the request stream
    // without necessarily emitting an 'error' — only 'close', after never
    // reaching 'end'. Without this, `out` never receives pipe()'s end() call
    // and the promise hangs forever. `reachedEnd` distinguishes this from a
    // normal completion, where 'close' always fires after 'end' per Node's
    // stream contract, so a resolved/settled promise is never re-rejected.
    readable.on('close', () => {
      if (!settled && !reachedEnd) cleanupAndReject(new Error('Upload aborted before completion'));
    });
...
function uploadBlob(ollamaCfg, filePath, digest, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Destroy without an error so this doesn't also fire the 'error' handler
    // above and get double-wrapped as "Cannot reach Ollama: ... timed out" —
    // a timeout doesn't mean Ollama is unreachable, just that this operation
    // ran past its window. Same approach as the existing Whisper handler.
    req.on('timeout', () => { req.destroy(); reject(new Error('Blob upload to Ollama timed out')); });
...
function createModel(ollamaCfg, modelName, filename, digest, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('error', (e) => reject(new Error(`Cannot reach Ollama: ${e.message}`)));
    req.on('timeout', () => { req.destroy(); reject(new Error('Model creation timed out')); });
    req.write(body);
    req.end();
  });
}
