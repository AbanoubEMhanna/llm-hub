...
async function checkHealth() {
  try {
    ...
    setCloudStatus('b-ai21',       data.providers?.ai21);
...
const MODEL_ID_PREFIX_RE = /^(ollama|lmstudio|openai|anthropic|groq|openrouter|mistral|together|fireworks|cohere|deepseek|cerebras|ai21|custom_[^/]+)\//;

function parseModelId(modelId) {
  const match    = (modelId || '').match(/^(ollama|lmstudio|openai|anthropic|groq|openrouter|mistral|together|fireworks|cohere|deepseek|cerebras|ai21|custom_[^/]+)\//);
...
function fillModelSelect(sel) {
  ...
  const ORDER = ['ollama', 'lmstudio', 'openai', 'anthropic', 'groq', 'openrouter', 'mistral', 'together', 'fireworks', 'cohere', 'deepseek', 'cerebras', 'ai21'];
...
function openApiKeySettings() {
  ...
  const keys = getStoredApiKeys();
  ['openai', 'anthropic', 'groq', 'openrouter', 'mistral', 'together', 'fireworks', 'cohere', 'deepseek', 'cerebras', 'ai21'].forEach(p => {
...
async function _runWizardDetection() {
  ...
  const KNOWN_LABELS = {
    ...
    deepseek: '☁️ DeepSeek', cerebras: '☁️ Cerebras', ai21: '☁️ AI21',
  };
